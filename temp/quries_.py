optimization_metrics = {
    "1. Wasted Compute (Failed/Cancelled Hours)": {
        ...
        """,
        "bad_user_pattern": "Users with >10 wasted hours/month are likely writing untested queries or have poor error handling",
        "action": "Code review, query testing in dev environment, timeout settings",
        "recommendation_step": "For users exceeding 10 wasted hours per month, initiate a targeted code review session focusing on their recent failed/cancelled queries. Provide guidance on effective error handling and emphasize the importance of thorough testing in a development environment before deploying to production. Consider implementing stricter timeout settings on their respective warehouses."
    },
    "2. Query Efficiency Ratio (Data Scanned vs Results)": {
        ...
        """,
        "bad_user_pattern": "Ratio >1000 indicates poor filtering, missing WHERE clauses, or full table scans",
        "action": "Implement clustering keys, add proper WHERE clauses, create summary tables",
        "recommendation_step": "For users with a scan-to-output ratio exceeding 1000, analyze their high-impact queries. Recommend implementing clustering keys on frequently filtered columns. Guide them on adding precise WHERE clauses to limit data scans and explore the creation of summary tables or materialized views for common analytical queries."
    },
    "3. Memory Pressure Index (Spill Rate)": {
        ...
        """,
        "bad_user_pattern": ">20% spill rate indicates complex joins without proper optimization",
        "action": "Optimize JOIN order, use appropriate warehouse size, implement query hints",
        "recommendation_step": "Address users exhibiting a spill rate above 20% by reviewing their complex join queries. Provide training on optimal JOIN order and the impact of warehouse sizing. Consider suggesting the use of query hints where applicable for specific problematic queries or temporarily increasing their warehouse size during peak analytical workloads."
    },
    "4. Compilation Overhead Score": {
        ...
        """,
        "bad_user_pattern": "Ratio >0.5 suggests dynamic SQL, frequent DDL, or schema changes",
        "action": "Use prepared statements, avoid dynamic SQL, implement query caching",
        "recommendation_step": "For users with a compile-to-execution ratio greater than 0.5, investigate their query patterns for excessive dynamic SQL or frequent DDL operations. Educate them on the benefits of prepared statements and encourage the use of query caching to reduce compilation overhead for repetitive queries."
    },
    "5. Concurrency Contention Impact": {
        ...
        """,
        "bad_user_pattern": "High queue times during business hours indicate poor query scheduling",
        "action": "Implement query scheduling, use separate warehouses for heavy workloads",
        "recommendation_step": "Address users experiencing average queue times exceeding 10 seconds during business hours by working with them to implement better query scheduling. For consistently heavy workloads, consider allocating a separate, appropriately sized warehouse to prevent contention and ensure smooth operation for other users."
    },
    "6. Data Hotspot Concentration": {
        ...
        """,
        "bad_user_pattern": "High concentration (>100) on few objects without result caching",
        "action": "Implement result caching, create materialized views, use query result cache",
        "recommendation_step": "For users with a high access concentration ratio (above 100) on a few specific objects, recommend leveraging Snowflake's result caching feature. Explore the creation of materialized views for their most frequently accessed and resource-intensive queries, and educate them on how the automatic query result cache can benefit their repetitive queries."
    },
    "7. Peak Hour Resource Abuse": {
        ...
        """,
        "bad_user_pattern": "Running >30min queries during 9-5 business hours",
        "action": "Schedule heavy queries for off-peak hours, implement query governance",
        "recommendation_step": "Engage with users consistently running queries longer than 30 minutes during peak business hours (9 AM - 5 PM). Advise them to schedule heavy data processing or reporting queries during off-peak hours. Implement query governance policies to restrict the execution of long-running queries during critical business periods."
    },
    "8. Error Pattern Frequency": {
        ...
        """,
        "bad_user_pattern": "Repeated same errors over multiple days",
        "action": "Targeted training, code review, implement error handling patterns",
        "recommendation_step": "For users exhibiting repeated errors with the same error codes or messages over multiple days, provide targeted training on common Snowflake error scenarios and debugging techniques. Conduct joint code reviews to identify underlying issues and assist them in implementing robust error handling patterns within their queries and scripts."
    }
}
...
optimization_charts = {
    "1. User Compute Cost Efficiency Matrix": {
        ...
        "bad_user_pattern": "Users in top-right quadrant (high compute, high data scan) need optimization",
        "recommendation_step": "Focus on users positioned in the top-right quadrant of the scatter plot, as they represent the highest compute cost and data scanned. Conduct a deep dive into their most expensive queries, identifying opportunities for query rewriting, data model optimization, or the introduction of materialized views to reduce scan volumes and compute time."
    },
    "2. Daily Waste Pattern Heatmap": {
        ...
        "show_table_toggle": True,
        "apply_object_filter": True,
        "bad_user_pattern": "Consistent daily waste indicates systemic issues",
        "recommendation_step": "Investigate users with consistently dark cells on the heatmap, indicating systemic issues leading to daily wasted compute. This may involve reviewing their standard operating procedures, automated scripts, or ETL processes for recurring errors. Provide tailored training or process improvements to address these underlying problems."
    },
    "3. Query Lifecycle Bottleneck Analysis": {
        ...
        "show_table_toggle": True,
        "apply_object_filter": True,
        "bad_user_pattern": "High compile time suggests dynamic SQL abuse",
        "recommendation_step": "For users with consistently high compilation times, investigate their use of dynamic SQL. Educate them on the performance benefits of parameterized queries and prepared statements. Encourage them to standardize their query patterns to allow Snowflake's query caching to be more effective."
    },
    "4. Spill Cascade Impact Chart": {
        ...
        "show_table_toggle": True,
        "apply_object_filter": True,
        "bad_user_pattern": "High spill rates in short queries indicate poor query design",
        "recommendation_step": "Address users whose short-duration queries (e.g., '<1min' or '1-5min' buckets) show high spill rates. This suggests inefficient query plans for even simple operations. Recommend reviewing their basic query constructs, especially for JOINs and aggregations, and ensuring appropriate filtering to reduce intermediate data sets."
    },
    "5. Hourly Resource Contention Map": {
        ...
        "show_table_toggle": True,
        "apply_object_filter": True,
        "bad_user_pattern": "Heavy usage during business hours (9-17) indicates poor scheduling",
        "recommendation_step": "Identify users with dark spots on the heatmap during business hours (9 AM - 5 PM). Work with these users to optimize their query execution times or to shift their non-critical, resource-intensive queries to off-peak hours. Consider implementing resource monitors for specific warehouses or users to proactively manage contention."
    },
    "6. Error Recovery Velocity": {
        ...
        "show_table_toggle": True,
        "apply_object_filter": True,
        "bad_user_pattern": "Increasing or flat error trends indicate learning issues",
        "recommendation_step": "Monitor users whose error trend lines are increasing or remaining flat. This suggests a lack of learning or persistent issues. Offer personalized coaching, provide access to relevant documentation or training materials, and encourage them to utilize Snowflake's support resources or internal experts for troubleshooting."
    },
    "7. Data Access Pattern Efficiency": {
        ...
        "show_table_toggle": True,
        "apply_object_filter": True,
        "bad_user_pattern": "High spread with low concentration indicates unfocused data access",
        "recommendation_step": "For users displaying a high 'Spread' (DATABASE_SPREAD, SCHEMA_SPREAD, TABLE_SPREAD) but low 'ACCESS_CONCENTRATION' on the radar chart, suggest refining their data access patterns. This might involve creating more specific views, understanding their data requirements better, or providing clearer guidance on the available data assets to prevent unfocused and potentially inefficient querying."
    },
...
response = {
    ...
    "key_optimization_actions_by_pattern": key_optimization_actions_by_pattern
}
